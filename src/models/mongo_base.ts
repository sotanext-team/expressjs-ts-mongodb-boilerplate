...
import { MESSAGES } from "@/libs/messages/types";
import { mergeDeep } from "@/libs/methods";
...
export class Model {
  ...
  ): Promise<{ total: number; list: IModel[] }> {
    const { page, size, sortType, ...query } = queryData;
    const setSize: number =
      size > config.paginator.maxPageSizeLimit
        ? config.paginator.maxPageSizeLimit
        : size;
    const sortBy: ISort =
      sortType && sortType !== config.paginator.sortTypes.date
        ? { [config.paginator.sortTypes[sortType]]: 1 }
        : { createdAt: -1 };
